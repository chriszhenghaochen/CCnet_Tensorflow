...
class vgg16(Network):
  ...
  def build_network(self, sess, is_training=True):
    with tf.variable_scope('vgg_16', 'vgg_16'):
      ...
      cls3_inds = tf.where(tf.less(cls3_prob[:,0], reject3))
      ...
      cls4_inds = tf.where(tf.less(cls4_prob[:,0], reject2))

      cls4_inds = tf.concat([cls4_inds, cls3_inds], 0)
      ...
      cls5_inds = tf.where(tf.less(cls5_prob[:,0], reject1))
      cls5_inds = tf.concat([cls5_inds, cls4_inds], 0)
      ...
      # total_inds = cls5_inds
      total_inds, _ = tf.unique(tf.reshape(cls5_inds,[-1]))
      self._predictions['total_inds'] = total_inds

      pool5 = tf.gather(pool5, tf.reshape(total_inds,[-1]))
      cls5_score = tf.gather(cls5_score, tf.reshape(total_inds,[-1]))

      ##################################training :reshape again###################################
      if is_training:          
        with tf.control_dependencies([rpn_labels]):
          rois_train, _, passinds = self._proposal_target_layer(rois, roi_scores, "rpn_rois", total_inds)
