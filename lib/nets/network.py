...
class Network(object):
  ...
  def _anchor_target_layer(self, rpn_cls_score, name, reject_inds):
    with tf.variable_scope(name) as scope:
      rpn_labels, rpn_bbox_targets, rpn_bbox_inside_weights, rpn_bbox_outside_weights = tf.py_func(
        anchor_target_layer,
        [rpn_cls_score, self._gt_boxes, self._im_info, self._feat_stride, self._anchors, self._num_anchors, reject_inds],
  ...
  def _proposal_target_layer(self, rois, roi_scores, name, passinds):
    with tf.variable_scope(name) as scope:
      rois, roi_scores, labels, bbox_targets, bbox_inside_weights, bbox_outside_weights, keep_inds = tf.py_func(
        proposal_target_layer,
        [rois, roi_scores, self._gt_boxes, self._num_classes, passinds],
  ...
  def DEBUG(self, sess, blobs):
    ...
    b1, b2, b3  = sess.run([self._predictions["total_inds"],
                            self._predictions["p5f"],
                            self._predictions["cls_prob_reshape"],
                           ],
                      feed_dict=feed_dict)
