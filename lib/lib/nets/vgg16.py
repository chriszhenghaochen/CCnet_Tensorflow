...
class vgg16(Network):
  ...
  def build_network(self, sess, is_training=True):
    with tf.variable_scope('vgg_16', 'vgg_16'):
      ...
      #combine
      scale_rpn3 = tf.Variable(tf.cast(1, tf.float32), trainable = is_training, name = 'scale_rpn3')
      ...
      rpn3_reject_inds = tf.where(tf.less(rpn3_cls_prob_reshape[:,0], rpn3_reject))

      if is_training:
        #compute anchor1 loss       
        rpn3_labels = self._anchor_target_layer(rpn3_cls_score, "anchor3", [], rpn_batch3, OHEM3)
      ##-----------------------------------------------rpn 2------------------------------------------------------------##
      ...
      #combine
      scale_rpn2 = tf.Variable(tf.cast(1, tf.float32), trainable = is_training, name = 'scale_rpn2')
      ...
      if is_training:
        #compute anchor1 loss       
        rpn2_labels = self._anchor_target_layer(rpn2_cls_score, "anchor2", rpn3_reject_inds, rpn_batch2, OHEM2)
      ##-----------------------------------------------rpn 1------------------------------------------------------------##
      # rpn 1
      rpn1 = slim.conv2d(self.endpoint['conv5_2'], 512, [3, 3], trainable=is_training, weights_initializer=initializer, scope="rpn1_conv/3x3")

      scale_rpn1 = tf.Variable(tf.cast(1, tf.float32), trainable = is_training, name = 'scale_rpn1')
      ...
      rpn1_reject_inds = tf.where(tf.greater(rpn1_cls_prob_reshape[:,0], rpn1_reject))
      rpn1_reject_inds = tf.concat([rpn2_reject_inds, rpn1_reject_inds],0)
      ...
      scale_rpn = tf.Variable(tf.cast(1, tf.float32), trainable = is_training, name = 'scale_rpn')
      ...
      if is_training:
        ...
        if cfg.TEST.MODE == 'nms':
          rois, _ = self._proposal_layer(rpn_cls_prob, rpn_bbox_pred, "rois", rpn1_reject_inds)
        elif cfg.TEST.MODE == 'top':
          rois, _ = self._proposal_top_layer(rpn_cls_prob, rpn_bbox_pred, "rois", rpn1_reject_inds)
        else:
      ...
      pool31_flat = slim.flatten(pool31_conv, scope='flatten31') 
      ...
      if is_training:          
        with tf.control_dependencies([rpn_labels]):
          rois_train3, _, passinds3 = self._proposal_target_layer(rois, roi_scores, "rpn3_rois", [], batch3)

          cls3_score_train = tf.gather(cls3_score, tf.reshape(passinds3,[-1]), name = 'cls3_score_train')
      ...
      pool41_flat = slim.flatten(pool41_conv, scope='flatten41') 
      ...
      cls4_score = slim.fully_connected(fc_combine4_2, self._num_classes, 
                                       weights_initializer=initializer,
                                       trainable=is_training,
                                       activation_fn=None, scope='cls4_score_pre')

      cls4_score = self._score_add_up(cls3_score, cls4_score, factor1, factor2, 'cls4_score')
      cls4_prob = self._softmax_layer(cls4_score, "cls3_prob")

      #generate target
      if is_training:          
        with tf.control_dependencies([rpn_labels]):
          rois_train4, _, passinds4 = self._proposal_target_layer(rois, roi_scores, "rpn2_rois", cls3_inds, batch2)

          cls4_score_train = tf.gather(cls4_score, tf.reshape(cls3_inds,[-1]))
          cls4_score_train = tf.gather(cls4_score_train, tf.reshape(passinds4,[-1]), name = 'cls4_score_train')
      ...
      pool51_flat = slim.flatten(pool51_conv, scope='flatten51') 
      ...
      cls5_score = slim.fully_connected(fc_combine5_2, self._num_classes, 
                                       weights_initializer=initializer,
                                       trainable=is_training,
                                       activation_fn=None, scope='cls5_score_pre')
      ...
      if is_training:          
        with tf.control_dependencies([rpn_labels]):
          rois_train5, _, passinds5 = self._proposal_target_layer(rois, roi_scores, "rpn1_rois", cls4_inds_uniq, batch1)

          cls5_score_train = tf.gather(cls5_score, tf.reshape(cls4_inds_uniq,[-1]))
          cls5_score_train = tf.gather(cls5_score_train, tf.reshape(passinds5,[-1]), name = 'cls5_score_train')
      ...
      cls_score = self._score_add_up(cls5_score, cls_score, factor1, factor2, 'cls_score')
      ...
      cls_prob = self._softmax_layer(cls5_score, "cls5_prob")
      cls_prob_reshape = tf.reshape(cls_prob, [-1, self._num_classes])
      ...
      self._predictions["cls_prob_reshape"] = cls_prob_reshape      
