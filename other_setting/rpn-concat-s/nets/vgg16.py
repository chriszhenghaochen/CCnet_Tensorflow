...
class vgg16(Network):
  ...
  def build_network(self, sess, is_training=True):
    with tf.variable_scope('vgg_16', 'vgg_16'):
      ...
      ##-----------------------------------------------rpn 1-----------------------------------------------------------------##
      conv4_resize = self._resize_map(self.endpoint['conv4_3'], self.endpoint['conv5_3'], "conv4_resize")
      rpn1 = slim.conv2d(conv4_resize, 512, [3, 3], trainable=is_training, weights_initializer=initializer, scope="rpn1_conv/3x3")

      # #normilize
      # rpn1_norm = self.normalize_to_target(rpn1, 10.0, 3, 'rpn1_norm', is_training)
      rpn1_scale = self.scale(rpn1, 0.1, 'rpn1_scale', is_training)

      #self._predictions['rpn1'] = rpn1
      ...
      #normilize
      # rpn_norm = self.normalize_to_target(rpn, 10.0, 3, 'rpn_norm', is_training)

      #concat
      rpn_concat = tf.concat([rpn, rpn1_scale], 3, 'rpn_concat')

      
      rpn_cls_score = slim.conv2d(rpn_concat, self._num_anchors * 2, [1, 1], trainable=is_training,
                                  weights_initializer=initializer,
                                  padding='VALID', activation_fn=None, scope='rpn_cls_score_pre')


      rpn_bbox_pred = slim.conv2d(rpn_concat, self._num_anchors * 4, [1, 1], trainable=is_training,
                                  weights_initializer=initializer,
      ...
      # pool41_avg = slim.avg_pool2d(pool41, [14, 14], padding='SAME', scope='pool41_avg', stride = 1)
      # pool41_flat = slim.flatten(pool41_avg, scope='flatten41')
      # fc4_2 = slim.fully_connected(pool41_flat, 512, scope='fc4_2', trainable=is_training)

      # #norm
      # pool41_norm = self.normalize_to_target(fc4_2, 10.0, 1, 'pool41_norm')
      ...
      # fc5_2 = slim.fully_connected(pool_avg_flat, 512, scope='fc5_2', trainable=is_training)
      # pool_norm = self.normalize_to_target(fc5_2, 10.0, 1, 'pool_norm')
