...
class vgg16(Network):
  ...
  def build_network(self, sess, is_training=True):
    with tf.variable_scope('vgg_16', 'vgg_16'):
      ...
      pool41_avg = slim.avg_pool2d(pool41, [7, 7], padding='VALID', scope='pool41_avg') 
      pool41_flat = slim.flatten(pool41_avg, scope='flatten41')
      pool41_norm = self.l2_norm(pool41_flat, 1, 1.0, 'pool41_norm', is_training)
      ...
      pool_avg = slim.avg_pool2d(pool5, [7, 7], padding='VALID', scope='pool_avg') 
      pool_avg_flat = slim.flatten(pool_avg, scope='pool_avg_flat')
      pool_norm = self.l2_norm(pool_avg_flat, 1, 1.0, 'pool_norm', is_training)
      ...
      # self._predictions['pool41_norm'] = pool_avg_flat
      # self._predictions['pool51_norm'] = pool41_flat


      fc_7_scale = self.scale(fc7, 1.0, 'fc7_scale', is_training)
      #fc7_norm = self.normalize_to_target(fc7, 10.0, 1, 'fc7_norm')

      fc7_concat = tf.concat([fc_7_scale, pool_norm, pool41_norm], 1, 'fc7_concat')
