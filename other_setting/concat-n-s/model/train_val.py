...
class SolverWrapper(object):
  ...
  def train_model(self, sess, max_iters):
    ...
    while iter < max_iters + 1:
      ...
      # plotarr_fc7.append(np.average(a, axis = 0))
      # plotarr_p5.append(np.average(b, axis = 0))
      # plotarr_p4.append(np.average(c, axis = 0))

      # if iter == 1000:
      #   fc7_p = np.average(np.array(plotarr_fc7), axis = 0) 
      #   p5_p = np.average(np.array(plotarr_p5), axis = 0) 
      #   p4_p = np.average(np.array(plotarr_p4), axis = 0) 

      #   import matplotlib.pyplot as pp
      #   val = fc7_p # this is the value where you want the data to appear on the y-axis.
      #   ar = np.arange(4096) # just as an example array
      #   pp.plot(ar, np.zeros_like(ar) + val, 'x')
      #   pp.show()

      #   val = p5_p # this is the value where you want the data to appear on the y-axis.
      #   ar = np.arange(512) # just as an example array
      #   pp.plot(ar, np.zeros_like(ar) + val, 'x')
      #   pp.show()

      #   val = p4_p # this is the value where you want the data to appear on the y-axis.
      #   ar = np.arange(512) # just as an example array
      #   pp.plot(ar, np.zeros_like(ar) + val, 'x')
      #   pp.show()
