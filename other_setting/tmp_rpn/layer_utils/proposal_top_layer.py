...
def proposal_top_layer(rpn_cls_prob, rpn_bbox_pred, im_info, _feat_stride, anchors, num_anchors, rpn_reject_inds_1, rpn_reject_inds_2):
  ...
  if rpn_reject_inds_1.size != 0:
    reject_inds_1 = np.unique(rpn_reject_inds_1)
    scores[reject_inds_1] = -2

  #reject via factor
  if rpn_reject_inds_2.size != 0:
    reject_inds_2 = np.unique(rpn_reject_inds_2)
