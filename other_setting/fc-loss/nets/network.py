...
class Network(object):
  ...
  def l2_norm(self,
    ...
    with tf.variable_scope(name) as scope:
      ...
      outputs = tf.nn.l2_normalize(inputs, dim)

      scale_name = name + '_scale_weight' 

      scale = tf.contrib.framework.model_variable(
          name=scale_name, dtype=tf.float32,
          initializer=tf.constant(scale_val, dtype=tf.float32),
          trainable=trainable)
  ...
  def DEBUG(self, sess, blobs):
    ...
    a = sess.run([self.endpoint['pool_concat']
  ...
  def _add_losses(self, sigma_rpn=3.0):
    with tf.variable_scope('loss_' + self._tag) as scope:
      ...
      #-----------------------------RCNN1, class loss-----------------------------------#
      cls1_score = self._predictions["cls4_score"]
      label1 = tf.reshape(self._proposal_targets["rpn_rois_labels"], [-1])
      ...
      #-----------------------------RCNN1_1, class loss-----------------------------------#
      cls_score_0 = self._predictions["cls5_score"]
      ...
      #total loss
      loss = cross_entropy + loss_box + rpn_cross_entropy + rpn_loss_box + cross_entropy1*0.5 + cross_entropy0
  ...
  def train_step(self, sess, blobs, train_op):
    feed_dict = {self._image: blobs['data'], self._im_info: blobs['im_info'],
                 self._gt_boxes: blobs['gt_boxes']}
    rpn_loss_cls, rpn_loss_box, loss_cls1, loss_cls0, loss_cls, loss_box, loss, _ = sess.run([self._losses["rpn_cross_entropy"],
                                                                        ...
                                                                        train_op],
                                                                       feed_dict=feed_dict)
    return rpn_loss_cls, rpn_loss_box, loss_cls1, loss_cls0, loss_cls, loss_box, loss

  def train_step_with_summary(self, sess, blobs, train_op):
    feed_dict = {self._image: blobs['data'], self._im_info: blobs['im_info'],
                 self._gt_boxes: blobs['gt_boxes']}
    rpn_loss_cls, rpn_loss_box, loss_cls1, loss_cls0, loss_cls, loss_box, loss, summary, _ = sess.run([self._losses["rpn_cross_entropy"],
                                                                                 self._losses['rpn_loss_box'],
                                                                                 self._losses['cross_entropy1'],
                                                                                 self._losses['cross_entropy0'],
                                                                                 self._losses['cross_entropy'],
                                                                                 ...
                                                                                 train_op],
                                                                                feed_dict=feed_dict)
    return rpn_loss_cls, rpn_loss_box, loss_cls1, loss_cls0, loss_cls, loss_box, loss, summary
