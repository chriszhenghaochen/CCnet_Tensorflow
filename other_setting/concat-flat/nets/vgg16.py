...
class vgg16(Network):
  ...
  def build_network(self, sess, is_training=True):
    with tf.variable_scope('vgg_16', 'vgg_16'):
      ...
      #l2 norm layer
      pool41_flat = slim.flatten(pool41, scope='pool41_flat')
      ...
      #l2 norm layer
      pool5_flat = slim.flatten(pool5, scope='pool5_flat')
      pool5_norm = self.normalize_to_target(pool5_flat, 10.0, 1, 'pool5_norm')

      pool_concat = tf.concat([pool41_norm, pool5_norm], 1, 'pool_concat')
      ...
      fc6 = slim.fully_connected(pool_concat, 4096, scope='fc6_1', trainable = is_training)
