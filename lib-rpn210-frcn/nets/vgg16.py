...
class vgg16(Network):
  ...
  def build_network(self, sess, is_training=True):
    with tf.variable_scope('vgg_16', 'vgg_16'):
      ...
      if is_training:          
        with tf.control_dependencies([rpn_labels]):
          rois_train4, _, passinds4 = self._proposal_target_layer(rois, roi_scores, "rpn2_rois", [], batch2)

          # cls4_score_train = tf.gather(cls4_score, tf.reshape(cls3_inds,[-1]))
          cls4_score_train = tf.gather(cls4_score, tf.reshape(passinds4,[-1]), name = 'cls4_score_train')
      ...
      if is_training:          
        with tf.control_dependencies([rpn_labels]):
          rois_train5, _, passinds5 = self._proposal_target_layer(rois, roi_scores, "rpn1_rois", cls4_inds, batch1)

          cls5_score_train = tf.gather(cls5_score, tf.reshape(cls4_inds,[-1]))
